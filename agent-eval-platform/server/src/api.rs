...
async fn create_profile(
    ...
    let valid_scaffolds = ["mock", "anthropic", "langgraph", "openai-agents", "mini-claude-code"];
    if !valid_scaffolds.contains(&req.scaffold.as_str()) {
...
async fn create_batch(
    ...
    for c in &req.cases {
        if c.get("case_id").and_then(|v| v.as_str()).is_none()
            || c.get("task").and_then(|v| v.as_str()).is_none()
        {
    ...
    tx.commit().await?;
    tracing::info!(batch_id = %batch_id, name = %req.name, cases = req.cases.len(), "batch created");
    Ok(Json(json!({ "id": batch_id })))
}

async fn list_batches(State(st): State<AppState>) -> ApiResult<Json<serde_json::Value>> {
    let rows: Vec<(Uuid, String, String, i64, i64, f64, chrono::DateTime<chrono::Utc>)> =
        sqlx::query_as(
...
async fn get_batch(
    ...
    let row: Option<(String, String, i32, chrono::DateTime<chrono::Utc>)> = sqlx::query_as(
        "SELECT name, status, parallelism, created_at FROM batches WHERE id = $1",
    )
    .bind(id)
    .fetch_optional(&st.db)
    .await?;
...
async fn cancel_batch(
    ...
    let mut tx = st.db.begin().await?;
    sqlx::query("UPDATE batches SET status = 'cancelled' WHERE id = $1 AND status IN ('pending','running')")
        .bind(id)
        .execute(&mut *tx)
        .await?;
    sqlx::query(
...
async fn list_runs(
    ...
    let rows: Vec<(Uuid, Uuid, String, String, Option<f32>, Option<f32>, Option<i32>)> =
        sqlx::query_as(
...
async fn dashboard_stats(State(st): State<AppState>) -> ApiResult<Json<serde_json::Value>> {
    ...
    let (queued,): (i64,) =
        sqlx::query_as("SELECT count(*) FROM runs WHERE status = 'queued'")
            .fetch_one(&st.db)
            .await?;
    let (active,): (i64,) =
    ...
    let status_map: std::collections::HashMap<String, i64> =
...
async fn compare_batches(
    ...
    if fp[0].2 != fp[1].2 {
        warnings.push(format!("harness_version 不同（{} vs {}）", fp[0].2, fp[1].2));
    }
    if fp[0].1 != fp[1].1 {
        warnings.push(format!("model 不同（{} vs {}）", fp[0].1, fp[1].1));
    }

    let rows: Vec<(String, String, String, Option<f32>, Option<f32>, Uuid, Uuid)> =
        sqlx::query_as(
...
async fn ingest_events(
    ...
    let batch_id: Option<(Uuid,)> = sqlx::query_as("SELECT batch_id FROM runs WHERE id = $1")
        .bind(id)
        .fetch_optional(&st.db)
        .await?;
    let batch_id = batch_id.ok_or(ApiError::NotFound("run"))?.0;
    ...
    for line in &lines {
        match serde_json::from_str::<TraceEvent>(line) {
            ...
            Err(e) => {
                tracing::warn!(run_id = %id, error = %e, "rejected malformed event");
                rejected += 1;
            }
        }
    }
    ...
    crate::metrics::events_ingested(accepted, rejected);
